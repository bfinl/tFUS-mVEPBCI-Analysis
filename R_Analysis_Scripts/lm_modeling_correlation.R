...
# Load in the data
# -----------------------------------------------------
data <- read_excel("/Users/jkosnoff/Library/CloudStorage/Box-Box/Joshua-Speller-Paper/Manuscript/nCommu/Source Data.xlsx", "Figure 4h")

# Only take the the data to be analyzed (the data file also contains p-values, which will mess up this script)
data <- data[, c("r","Condition","Subject")]
data$Subject <- factor(data$Subject)
data$Condition <- factor(data$Condition)

# Fit the model
lmm <- lmer(r ~ Condition + (1|Subject), 
            data = data)
